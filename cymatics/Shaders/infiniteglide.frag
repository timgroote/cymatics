...
vec3 drawSquare (in vec2 p, in vec2 square, in vec3 setting) {
    if(boxtest(p, square, setting.x) && !(boxtest(p, square, setting.x - setting.y)))
        return palette(setting.z / 70.0 + iGlobalTime * 0.1);
...
void main (void) {
    ...
    float a = sin(iGlobalTime) * 0.6; //box angle
    mat2 rot = mat2(cos(a), -sin(a), sin(a), cos(a));
    pos = uv * aspect * rot;
    center *= aspect * rot;

    /*for(int i = 0; i < 50; i++) {
        vec3 d = drawSquare(pos, center + vec2(sin(float(i) / 10.0 + iGlobalTime) / 4.0, 0.0), vec3(0.0 + sin(float(i) / 200.0), 0.01 , float(i)));
